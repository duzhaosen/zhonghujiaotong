...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="LineRightBlue1">
  <tr>
    <td width="88%" height="24"><a class="tab_top_btn" href="<?php echo U('Link/add');?>">添加链接</a></td>
...
<form name="theForm" method="post" action="<?php echo U('uporder');?>">
  ...
  <div id="languageBox2">
    <div class="tablelist">
    <?php if(is_array($links)): $i = 0; $__LIST__ = $links;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul onmouseover="this.style.background ='#FFFDD7'" onmouseout="this.style.background ='#FFFFFF'">
          <li class="li_10">
            <input type="text" value="<?php echo ($vo["sort"]); ?>" name="sort[<?php echo ($vo["id"]); ?>]" class="listinput"/>
          </li>
          <li class="li_30"><?php echo ($vo["name"]); ?></li>
